...
1. **Asset register** — every asset individually tagged under the client's own numeric tag scheme (tag assigned at delivery, so records can exist untagged while on order); categories covering *any* IT asset (laptops, phones, desktops, printers, mice, headphones, etc.); purchase data (date, price, supplier, warranty); status lifecycle including the repair loop; site/location.
2. **Assignment** — assign/return to staff with full immutable history; per-asset and per-person views.
3. **Migration and reporting** — import of the client's Asset Tiger backup/export; reports and CSV export, including a finance-shaped export.
...
1. **Asset Tiger cancelled before its new pricing model applies** — the primary, deadline-driven metric.
2. All ~400 assets migrated with tags, statuses, and assignees intact (verified against Asset Tiger totals).
...
1. Exact Asset Tiger annual cost — still unstated after two direct asks. Estimated $240–600/year from published tiers. Only affects the economics narrative, not the build.
...
5. Asset Tiger export schema — client holds a backup; inspect in week 1 to de-risk the import story.
...
- Asset Tiger (current system; backup/export in client's hands) — https://www.assettiger.com
...
- **Immediate client actions:** (1) take a fresh, full Asset Tiger export *now* — before any pricing/access change; (2) confirm the pricing-change effective date; (3) confirm whether "ID" means national ID, and whether an employee number can serve instead.
